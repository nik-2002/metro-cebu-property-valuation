"""Build defense_deck_v3.pptx from the v3 deck content (build_deck_v3.SLIDES).
...
import build_deck_v3 as deck  # importing builds SLIDES (and harmlessly rewrites the HTML)
...
def r_claim(s, n, x, y, w):                 # centered, narrowed main-claim statement
    cw = w * 0.74; cx = x + (w - cw) / 2
    h = est_h(node_text(n), 26, cw, lh=1.32, pad=0.22)
    tf = box(s, cx, y, cw, h); p = tf.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER; p.line_spacing = 1.32
    add_runs(p, n, color=INK, size=26)
    return h + 0.12
...
def r_blocks(s, kids, x, y, w, bottom):
    ...
    while i < len(kids):
        ...
        elif n.tag == "div" and "claim" in c:       cy += r_claim(s, n, x, cy, w)
...
def est_one(n, w):
    ...
    if t == "div" and "claim" in c:     return est_h(node_text(n), 26, w * 0.74, lh=1.32, pad=0.22) + 0.12
...
def main():
    ...
    out = os.path.join(HERE, "defense_deck_v3.pptx")
    prs.save(out)
    print(f"wrote defense_deck_v3.pptx  ({n} slides)")
