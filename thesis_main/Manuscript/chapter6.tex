\section{Chapter 6: Modeling}\label{modeling}
...
\subsection{Model Selection Rationale}\label{chapter6-model-selection-rationale}
...
\subsection{Stratified Fitting and Leak-Free Validation}\label{stratified-fitting}
...
Table~\ref{tab:chapter6-split} summarizes the stratified modeling setup across the three property strata.
...
\subsection{Per-Stratum Feature Sets}\label{per-stratum-feature-sets}
...
\subsection{OLS Hedonic Baseline}\label{ols-hedonic-baseline}
...
\end{tabular}
\end{center}
\tablenote{Maximum depth was left unbounded in all three strata.}
\end{table}
...
\subsection{Hyperparameter Tuning}\label{hyperparameter-tuning-ch6}
...
\subsection{MCRAI Composite Weight Derivation}\label{mcrai-stage2-weight-derivation}

The MCRAI composite was built from the OLS coefficient signs rather than by forcing all categories into one additive score. It retained only the positive-coefficient household-access categories and rescaled their implicit prices into a three-part weight vector: 0.447 for education, 0.345 for grocery, and 0.222 for recreation. Transport accessibility was represented separately through the road-network distance features rather than as a composite category, and finance was retired from the framework.
...
Table~\ref{tab:chapter6-mcrai-weights} lists the resulting composite weights for the positive-coefficient categories.
...
\subsection{SHAP Setup}\label{shap-setup}
