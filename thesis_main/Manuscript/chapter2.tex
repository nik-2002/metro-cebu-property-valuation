\section{Chapter 2: Review of Related Literature}\label{review-of-related-literature}
...
\subsection{Core Concepts and Philippine Context}\label{core-concepts-and-philippine-context}

\subsubsection{Valuation Foundations}\label{valuation-foundations}
...
\subsubsection{Cebu-Specific Empirical Work}\label{cebu-specific-empirical-work}
...
\subsection{The Core Problem: Data Scarcity in Emerging Markets}\label{the-core-problem-data-scarcity-in-emerging-markets}
...
\subsection{Modeling Approaches: Traditional vs.~Machine Learning}\label{modeling-approaches-traditional-vs.-machine-learning}

\subsubsection{Hedonic Regression}\label{hedonic-regression}
...
\subsubsection{Philippine Machine Learning Studies}\label{philippine-machine-learning-studies}
...
\subsubsection{International Evidence: The Tanzania Benchmark}\label{international-evidence-the-tanzania-benchmark}

The most directly relevant international study is \textcite{nyanda2024}, who tested eight ML algorithms on approximately 954 residential properties in Dar es Salaam---a sample size close to that of the individual property strata in this study (condominiums 1{,}300, houses 1{,}223, and vacant lots 849). Table~\ref{tab:chapter2-tanzania-benchmark} reports the MAPE they obtained for each algorithm.
...
\subsubsection{Southeast Asian Applications}\label{southeast-asian-applications}

Work from Southeast Asia points in a similar direction. In Indonesia, \textcite{tanamal2023} used a Random Forest model to sort Surabaya houses into price categories from a set of agent-sourced attributes, including land and building area, number of rooms, road width, building age, and area classification, and reported that the tree-based approach handled this mixed attribute data well. In Malaysia, \textcite{jaafar2021} showed that Random Forest best captured the value contribution of heritage characteristics for pre-war shophouses in Penang, outperforming more conventional regression approaches. These studies come from different property contexts, but together they suggest that tree-based models are well suited to the varied attribute data that residential valuation depends on in fast-changing urban markets.

\subsubsection{Comparative Reviews}\label{comparative-reviews}
...
\subsection{Geospatial Feature Engineering in Property Valuation}\label{geospatial-feature-engineering-in-property-valuation}
...
\subsubsection{Geocoding and Location Precision}\label{geocoding-and-location-precision}
...
\subsubsection{Proximity Analysis and Accessibility}\label{proximity-analysis-and-accessibility}
...
\subsubsection{Accessibility Indexing and OpenStreetMap Support}\label{amenity-scoring-via-openstreetmap}
...
\subsubsection{Spatial Autocorrelation and Neighbor Price Effects}\label{spatial-autocorrelation-and-neighbor-price-effects}
...
% Ms. Kim: remove the extra space before this enumeration (list top-separation).
...
\subsubsection{Implications for Metro Cebu}\label{implications-for-metro-cebu}
...
\subsection{Integrating Value Drivers: Indexing vs.~Raw Features}\label{integrating-value-drivers-indexing-vs.-raw-features}
...
\subsection{Macroeconomic Factors and Study Scope}\label{macroeconomic-factors-and-study-scope}

Broad macroeconomic conditions, such as exchange rates, inflation, and interest rates, influence the direction of a housing market over time. This study does not model those forces. Its design is cross-sectional: it explains differences in price per square meter across locations at a single point in time, using property attributes and spatial features rather than market-wide cycles. Macroeconomic determinants therefore fall outside the scope of the present model. They remain a sensible direction for later work, and Chapter 10 records them as a recommendation for further research.


\subsection{Compliance and Explainability: IVS 2025}\label{compliance-and-explainability-ivs-2025}
...
% Ms. Kim: remove the extra space before this list (list top-separation).
\begingroup\setlength{\topsep}{0pt}\setlength{\partopsep}{0pt}
\begin{itemize}
...
\subsection{Synthesis and Research Gap}\label{synthesis-and-research-gap}
...
\subsection{Bridge to Methodology}\label{bridge-to-methodology}
