...
The fourth recommendation is to use the valuation-gap output as a screening device for institutional review. The thesis did not reduce the valuation gap to one metro-wide statistic, but it did preserve the gap as a diagnostic field in the analytical base table and map outputs. This makes it useful for flagging places where market-facing estimates and official BIR benchmarks appear to diverge enough to justify closer appraisal review.
...
The first deployment recommendation is to keep the Streamlit app tied to the project virtual environment so that all dependencies, including SHAP, are available at runtime. The app should be launched through the project environment rather than the system Python. A simple launcher script or startup note that activates the correct environment before running the app is sufficient for most handoff scenarios.
...
The third recommendation is to make the model scope explicit everywhere a user sees the app. The price surface predicts open-market residential levels only. It does not predict bank ROPA liquidation values, government floor prices, or every possible property segment in the ABT. That scope should appear clearly in the interface, documentation, and demo script so users do not read the surface as a universal market-value map.
...
The fifth recommendation is to keep the Streamlit app and QGIS outputs consistent with each other. Both deliverables draw from the same saved model artifact and the same prediction export. Whenever the model is retrained or the ABT is updated, the prediction layer should be regenerated and verified against both the app and the QGIS map before either is shown externally.
