...
The open-market layer was itself built from several listing portals rather than a single site, which widened coverage and reduced dependence on any one platform's inventory. Table~\ref{tab:chapter3-data-sources} summarizes the sources and their roles.
...
\midrule
\textbf{Open-Market Listings} & Deployed training label source & Lamudi, FilipinoHomes, DotProperty (3{,}616 open-market rows) & Asking / market-facing \\
...
These distressed assets were treated as a lower-bound segment of the observed market rather than as direct equivalents of open-market transaction prices. They were kept in the wider data system as reference layers and were not used as training labels for the deployed open-market model. A sample of the raw BDO and bank ROPA input structure is documented in Appendix A.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{diagrams/Data-Pipeline-Updated.png}
\caption{Data pipeline from multi-source ingestion to the modeling-ready Analytics Base Table.}
\label{fig:data-pipeline}
\end{figure}

\begin{enumerate}
\item \textbf{Ingestion}: Open-market listings were collected from Lamudi (bulk \emph{requests} scraper plus a Playwright browser scraper), FilipinoHomes (backend JSON interface), and DotProperty (portal listings). Bank foreclosure data was ingested from Excel via Pandas, and Pag-IBIG floor-price records from structured PDFs and Excel exports.
\item \textbf{Filtering}: The dataset was restricted to residential properties within Metro Cebu---Cebu City, Mandaue City, Lapu-Lapu City, Talisay City, Minglanilla, and Consolacion---using point-in-polygon checks against the six LGU boundaries. The canonical ABT retained all three market tiers, but the modeling scripts filtered to \emph{open\_market} observations only before fitting.
...
\item \textbf{Final ABT Assembly and Stratum Split}: All engineered features were merged into a single Analytics Base Table and saved as a flat CSV. The modeling-ready subset was produced by filtering to \emph{open\_market}, dropping rows with null \emph{price\_per\_sqm} or \emph{spatial\_lag\_price}, applying integrity flags, and then splitting into the condominium, house, and vacant-lot strata for separate model fitting.
\end{enumerate}
