\section{Chapter 5: Data Preparation}\label{data-preparation}
...
\subsection{Scope of Preparation Work}\label{scope-of-preparation-work}
...
\subsection{Cleaning and Filtering}\label{cleaning-and-filtering}
...
\subsection{Stratum Split}\label{stratum-split}
...
\end{tabular}
\end{center}
\textit{Note.} The three strata sum to the 3{,}372-row modeling subset used to fit the models; the remaining 244 of the 3{,}616 assembled rows were held out by the model-requirement filters (see below).
\end{table}

The 244-row gap between the assembled ABT and the modeling subset comes from three model-requirement filters applied at fitting: rows with no computable \emph{price\_per\_sqm} target, rows with no same-type neighbor within 500 m for the spatial-lag feature, and vacant lots outside the lot-scope band. These rows are clean but cannot serve as training examples, so they stay in the assembled ABT for description only.
...
\subsection{Missing Data Treatment}\label{missing-data-treatment}
...
\subsection{Market Segment Treatment}\label{market-segment-treatment}
...
\subsection{Feature Engineering}\label{feature-engineering-ch5}
...
\subsection{Encoding and Final Modeling-Ready Matrices}\label{encoding-and-final-modeling-ready-dataset}
