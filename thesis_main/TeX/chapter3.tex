...
\begin{itemize}
    \item \textbf{Web Scraping}: \texttt{Requests} and \texttt{BeautifulSoup} \parencite{richardson2007beautiful} alongside multi-threading libraries to extract JSON-LD structured data from online property platforms.
...
\begin{enumerate}
    \item \textbf{Data Ingestion \& Scraping}:
    \begin{itemize}
        \item \textit{BDO Foreclosed Properties}: The official baseline data is ingested directly from the BDO Excel file into the Python environment.
        \item \textit{Lamudi Online Listings}: To establish the market ceiling price, property listings are scraped from Lamudi. This is achieved using a multi-threaded Python pipeline comprising \texttt{Requests} \parencite{reitz2011requests} and \texttt{BeautifulSoup}. The scraper navigates paginated search results to extract individual property URLs, and then accesses each page to extract structured JSON-LD data (encompassing price, lot area, floor area, bedrooms, bathrooms, and geolocation coordinates).
    \end{itemize}
    \item \textbf{Filtering}: The datasets are filtered to include only "Residential" properties located in "Cebu City" or key Metro Cebu cities (Mandaue, Lapu-Lapu, Talisay, Minglanilla, Consolacion).
