...
Taken together, these studies show that there is still no Cebu-focused, multi-source model that works at the property level and is easy to run. RREPI/RPPI give trends. REPI sketches a national framework. Prior Philippine studies using machine learning are either limited in coverage, rely on small manual datasets, or focus on other cities. None of them deliver an open, repeatable pipeline—from automated web scraping to valuation—for Cebu pricing \parencite{bsp2025, domingo2005}. Our thesis responds to this by designing and testing a practical valuation model for Cebu that follows these lessons but is grounded in large-scale, locally scraped, and verified data.
